#!/usr/bin/env python3
"""
Batch: load clean WAVs → AWGN → Butterworth high-pass → SNR vs clean → CSV/JSON report.

Usage:

    python run_batch_highpass_awgn.py
    python run_batch_highpass_awgn.py --cutoff-hz 80 --target-snr-db 0 --no-audio
"""
...
def main() -> None:
    root = repo_root()
    p = argparse.ArgumentParser(description="AWGN + high-pass batch on data/clean")
    p.add_argument("--input-dir", type=Path, default=root / "data/clean")
    p.add_argument("--output-dir", type=Path, default=root / "data/output/batch_highpass_awgn")
    ...
    p.add_argument("--cutoff-hz", type=float, default=80.0)
    ...
    for i, wav in enumerate(wavs):
        ...
        try:
            ...
            nyq = sr / 2.0
            if args.cutoff_hz <= 0 or args.cutoff_hz >= nyq:
                raise ValueError(f"cutoff_hz must be in (0, Nyquist={nyq})")
            rng = np.random.default_rng(args.seed + i * 1_000_003)
            noisy = clean + make_awgn(clean, args.target_snr_db, rng)
            t0 = time.perf_counter()
            denoised = butter_highpass(noisy, sr, args.cutoff_hz, args.order)
            ...
            if not args.no_audio:
                ...
                out_path = output_dir / f"{wav.stem}-snr{args.target_snr_db:g}-hp{args.cutoff_hz:g}Hz-o{args.order}-denoised.wav"
    ...
    csv_p, json_p = write_eval_reports(rows, eval_dir, "batch_highpass_awgn_eval")
    if not args.quiet:
        print(f"Wrote {csv_p}\nWrote {json_p}")
