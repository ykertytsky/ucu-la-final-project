#!/usr/bin/env python3
"""
Batch: load clean WAVs → AWGN → Butterworth band-pass → SNR vs clean → CSV/JSON report.

Usage:

    python run_batch_bandpass_awgn.py
    python run_batch_bandpass_awgn.py --low-hz 80 --high-hz 7000 --target-snr-db 0 --no-audio
"""
...
def main() -> None:
    root = repo_root()
    p = argparse.ArgumentParser(description="AWGN + band-pass batch on data/clean")
    p.add_argument("--input-dir", type=Path, default=root / "data/clean")
    p.add_argument("--output-dir", type=Path, default=root / "data/output/batch_bandpass_awgn")
    ...
    p.add_argument("--high-hz", type=float, default=7000.0)
    ...
    for i, wav in enumerate(wavs):
        ...
        try:
            ...
            nyq = sr / 2.0
            hi = min(args.high_hz, nyq * 0.99)
            if args.low_hz <= 0 or args.low_hz >= hi:
                raise ValueError(f"need 0 < low_hz < high_hz <= ~Nyquist; got low={args.low_hz} high={hi}")
            rng = np.random.default_rng(args.seed + i * 1_000_003)
            noisy = clean + make_awgn(clean, args.target_snr_db, rng)
            t0 = time.perf_counter()
            denoised = butter_bandpass(noisy, sr, args.low_hz, hi, args.order)
            row["time_filter_s"] = time.perf_counter() - t0
            row["high_hz_effective"] = float(hi)
            ...
            if not args.no_audio:
                ...
                out_path = (
                    output_dir
                    / f"{wav.stem}-snr{args.target_snr_db:g}-bp{args.low_hz:g}-{hi:g}Hz-o{args.order}-denoised.wav"
                )
    ...
    csv_p, json_p = write_eval_reports(rows, eval_dir, "batch_bandpass_awgn_eval")
    if not args.quiet:
        print(f"Wrote {csv_p}\nWrote {json_p}")
