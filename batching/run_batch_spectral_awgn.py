#!/usr/bin/env python3
"""
Batch: load clean WAVs → AWGN → spectral subtraction (STFT) → SNR vs clean → CSV/JSON.

Same spectral subtraction idea as other_methods.ipynb (leading frames as noise probe).

Usage:

    python run_batch_spectral_awgn.py
    python run_batch_spectral_awgn.py --noise-threshold 2.5 --no-audio
"""
...
def main() -> None:
    root = repo_root()
    p = argparse.ArgumentParser(description="AWGN + spectral subtraction batch on data/clean")
    p.add_argument("--input-dir", type=Path, default=root / "data/clean")
    p.add_argument("--output-dir", type=Path, default=root / "data/output/batch_spectral_awgn")
    ...
    p.add_argument("--noise-threshold", type=float, default=2.0)
    p.add_argument("--noise-cols", type=int, default=None, help="STFT frames for noise estimate (default: sr*0.5/hop)")
    ...
    for i, wav in enumerate(wavs):
        ...
        try:
            ...
            denoised = spectral_denoise_waveform(
                noisy,
                sr,
                n_fft=args.n_fft,
                hop_length=args.hop_length,
            ...
            row["time_spectral_s"] = time.perf_counter() - t0
            ...
            if not args.no_audio:
                ...
                out_path = output_dir / f"{wav.stem}-snr{args.target_snr_db:g}-spec-denoised.wav"
    ...
    csv_p, json_p = write_eval_reports(rows, eval_dir, "batch_spectral_awgn_eval")
    if not args.quiet:
        print(f"Wrote {csv_p}\nWrote {json_p}")
